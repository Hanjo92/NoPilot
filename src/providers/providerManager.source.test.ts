...
test('provider quick pick is backed by unified model entries', () => {
  ...
  assert.match(source, /const providerItems: ProviderQuickPickItem\[\] = this\.getAllProviderInfos\(\)\.map/);
  assert.match(source, /title: 'NoPilot: Select Provider'/);
  assert.match(source, /const modelEntries = this\.buildModelEntries\(\)\.filter\(\(entry\) =>/);
  assert.match(source, /entry\.providerId === selectedProvider\.providerId && entry\.modelKey/);
  assert.match(source, /title: `NoPilot: Select Model · \$\{providerInfo\.name\}`/);
  assert.match(source, /if \(selectedProvider\.providerId && modelEntries\.length === 0\) \{/);
  ...
  assert.match(source, /if \(selectedModel\.providerId && selectedModel\.modelKey\) \{/);
  assert.match(source, /await this\.switchTo\(selectedModel\.providerId, selectedModel\.modelKey\)/);
...
test('provider quick pick keeps most-used summary out of option rows', () => {
  ...
  assert.match(source, /placeHolder: `Choose a provider · \$\{this\.getProviderUsageSummaryLabel\(\)\}`/);
  assert.match(source, /placeHolder: `Choose a model from \$\{providerInfo\.name\}`/);
});
